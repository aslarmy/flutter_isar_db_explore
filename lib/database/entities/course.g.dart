...
extension CourseByIndex on IsarCollection<Course> {
  Future<Course?> getByTitle(String title) {
    return getByIndex(r'title', [title]);
  }

  Course? getByTitleSync(String title) {
  ...
  Future<List<Course?>> getAllByTitle(List<String> titleValues) {
    final values = titleValues.map((e) => [e]).toList();
    return getAllByIndex(r'title', values);
  }

  List<Course?> getAllByTitleSync(List<String> titleValues) {
    final values = titleValues.map((e) => [e]).toList();
  ...
  Future<Id> putByTitle(Course object) {
    return putByIndex(r'title', object);
  }

  Id putByTitleSync(Course object, {bool saveLinks = true}) {
    return putByIndexSync(r'title', object, saveLinks: saveLinks);
  }

  Future<List<Id>> putAllByTitle(List<Course> objects) {
    return putAllByIndex(r'title', objects);
  }

  List<Id> putAllByTitleSync(List<Course> objects, {bool saveLinks = true}) {
...
extension CourseQueryWhere on QueryBuilder<Course, Course, QWhereClause> {
  ...
  QueryBuilder<Course, Course, QAfterWhereClause> titleEqualTo(String title) {
    return QueryBuilder.apply(this, (query) {
  ...
  QueryBuilder<Course, Course, QAfterWhereClause> titleNotEqualTo(
      String title) {
    return QueryBuilder.apply(this, (query) {
      if (query.whereSort == Sort.asc) {
        return query
